(function() {
  var AASM, Event, StateMachine, capitalize, _callAction, _ref;
  var __slice = Array.prototype.slice;

  _ref = require('./helpers'), capitalize = _ref.capitalize, _callAction = _ref._callAction;
  ...
  module.exports = AASM = (function() {
    ...
    ClassMethods = {
      ...
      aasmState: function(name, options) {
        ...
        if (!sm.initialState) sm.initialState = name;
      ...
      aasmEvent: function(name, options, block) {
        ...
        sm = StateMachine[this];
        if (!sm.events[name]) sm.events[name] = new Event(name, options, block);
    ...
    PrototypeMethods = {
      aasmCurrentState: function() {
        if (this._aasmCurrentState) return this._aasmCurrentState;
      ...
      aasmEventsForState: function(state) {
        ...
        values = (function() {
          var _ref2, _results;
          _ref2 = this.constructor.aasmEvents();
          _results = [];
          for (name in _ref2) {
            value = _ref2[name];
            _results.push(value);
      ...
      setAasmCurrentStateWithPersistence: function(state) {
        var saveSuccess;
        saveSuccess = true;
        if (this.aasmWriteState != null) saveSuccess = this.aasmWriteState(state);
        if (saveSuccess) this._aasmCurrentState = state;
        return saveSuccess;
      },
        ...
        try {
          ...
          if (newStateName !== null) {
            ...
            if (persist) {
              ...
              if (persistSuccessful) event.executeSuccessCallback(this);
            } else {
            ...
            if (persistSuccessful) {
              ...
              if (this.aasmEventFailed) this.aasmEventFailed(name, oldState.name);
            }
